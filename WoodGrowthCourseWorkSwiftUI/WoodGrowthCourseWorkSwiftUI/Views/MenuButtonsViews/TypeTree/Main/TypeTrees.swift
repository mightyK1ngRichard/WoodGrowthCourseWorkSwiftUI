...
struct TypeTrees: View {
    ...
    private func getTreesInThisPlot(_ typeId: String, completion: @escaping ([TreeResult]?, String?) -> Void) {
        APIManager.shared.getTrees(typeID: typeId) { data, error in
            ...
            for el in data.rows {
                let info = TreeResult(id: el.tree_id, name_tree: el.name_tree, volume: el.volume, date_measurements: el.date_measurements, notes: el.notes, name_type: el.name_type, name_plot: el.name_plot, x_begin: el.x_begin, x_end: el.x_end, y_begin: el.y_begin, y_end: el.y_end, photo: el.photo, typeID: "0")
